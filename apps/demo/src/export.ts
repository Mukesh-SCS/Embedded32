...
export type TraceValidationResult =
  { ok: true; trace: Trace } | { ok: false; error: string; field?: string };

function validateFrame(
  frame: unknown,
  index: number
): { ok: true; frame: TraceFrame } | { ok: false; error: string; field?: string } {
  ...
  if (
    typeof f.timestampMs !== 'number' ||
    !Number.isFinite(f.timestampMs) ||
    f.timestampMs < 0 ||
    !Number.isInteger(f.timestampMs)
  ) {
    return {
      ok: false,
      error: `Frame ${index}: timestampMs must be a non-negative integer`,
      field: `frames[${index}].timestampMs`,
    };
  }
  ...
  if (!Array.isArray(f.data)) {
    return {
      ok: false,
      error: `Frame ${index}: data must be an array`,
      field: `frames[${index}].data`,
    };
  }
  ...
  if (f.data.length > 8) {
    return {
      ok: false,
      error: `Frame ${index}: data must contain at most 8 bytes`,
      field: `frames[${index}].data`,
    };
  }
  ...
  for (let b = 0; b < f.data.length; b++) {
    ...
    if (typeof byte !== 'number' || !Number.isInteger(byte) || byte < 0 || byte > 255) {
      return {
        ok: false,
        error: `Frame ${index}: byte ${b} must be an integer 0-255`,
        field: `frames[${index}].data[${b}]`,
      };
    }
  }
...
export function validateTraceInput(raw: string): TraceValidationResult {
  ...
  if (new TextEncoder().encode(raw).length > MAX_TRACE_BYTES) {
    return {
      ok: false,
      error: `Trace exceeds maximum size of ${MAX_TRACE_BYTES} bytes`,
      field: 'trace',
    };
  }
  ...
  if (candidate.scenario.length > MAX_SCENARIO_LENGTH) {
    return {
      ok: false,
      error: `scenario must be at most ${MAX_SCENARIO_LENGTH} characters`,
      field: 'scenario',
    };
  }
