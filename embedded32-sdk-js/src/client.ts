...
export class J1939Client extends EventEmitter implements IJ1939Client {
  ...
  async connect(): Promise<void> {
    ...
    this.log(
      `Connecting to ${this.config.interface} as SA=0x${this.config.sourceAddress.toString(16).toUpperCase()}`
    );
  ...
  async requestPGN(pgn: number, destination: number = 0xff): Promise<void> {
    ...
    this.log(
      `Requesting PGN 0x${pgn.toString(16).toUpperCase()} from SA=0x${destination.toString(16).toUpperCase()}`
    );
  ...
  async sendPGN(pgn: number, data: PGNData, destination: number = 0xff): Promise<void> {
    ...
    this.log(
      `Sending PGN 0x${pgn.toString(16).toUpperCase()} to SA=0x${destination.toString(16).toUpperCase()}`
    );
