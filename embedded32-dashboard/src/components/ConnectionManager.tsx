...
const ConnectionManager: React.FC = () => {
  ...
  const [url, setUrl] = useState(storedUrl && storedUrl.includes('9001') ? storedUrl : defaultUrl);
    ...
    <div className="card">
      ...
      <div className="card-body" style={{ display: 'flex', gap: 8, flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-primary" onClick={connect} disabled={state.connected}>
            Connect
          </button>
          <button className="btn btn-outline" onClick={disconnect} disabled={!state.connected}>
            Disconnect
          </button>
        </div>
      </div>
