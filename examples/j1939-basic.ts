...
async function basicJ1939Example(): Promise<void> {
  ...
  console.log(
    `  Source Address:  0x${parsed.sourceAddress.toString(16).toUpperCase().padStart(2, '0')}`
  );
  ...
  console.log(
    `Raw Data: ${exampleData.map((b) => b.toString(16).toUpperCase().padStart(2, '0')).join(' ')}`
  );
  ...
  can.onFrame((frame: any) => {
    receivedCount++;
    console.log(
      `  Frame ${receivedCount}: ID=0x${frame.id.toString(16).toUpperCase()}, Data=${frame.data.join(',')}`
    );
